...
// Función para inicializar o restablecer la geometría limpia
function initPentagono() {
    pentagonoVertices = [];
    // Generar los 5 vértices de la cara frontal (Z positivo)
    for (let i = 0; i < 5; i++) {
        // Usamos +90 para que la punta matemática quede orientada hacia arriba en el canvas
    ...
    // Generar los 5 vértices de la cara trasera (Z negativo)
    for (let i = 0; i < 5; i++) {
        let angulo = (i * 72 + 90) * Math.PI / 180;
        pentagonoVertices.push({ x: radioBase * Math.cos(angulo), y: radioBase * Math.sin(angulo), z: -profundidad });
    }
...
// Se ejecuta la primera carga inicial del arreglo geométrico
initPentagono();

// Definición estructural de las 7 caras del prisma
const pentagonoFaces = [
  { indices: [0, 1, 2, 3, 4], color: 'red' },      // Cara frontal
  { indices: [9, 8, 7, 6, 5], color: 'blue' },     // Cara trasera
  { indices: [0, 1, 6, 5], color: 'green' },       // Lateral 1
  { indices: [1, 2, 7, 6], color: 'yellow' },      // Lateral 2
  { indices: [2, 3, 8, 7], color: 'orange' },      // Lateral 3
  { indices: [3, 4, 9, 8], color: 'purple' },      // Lateral 4
  { indices: [4, 0, 5, 9], color: 'cyan' }         // Lateral 5
];
...
function drawPentagono() {
    ...
    // Algoritmo del pintor (ordenar por profundidad Z media)
...
// Función que limpia transformaciones previas, reorienta y renderiza de cero
